...
module.exports = {
    up: async (queryInterface, Sequelize) => {
        ...
        await queryInterface.bulkInsert('Courses', courses.map(({tags, ...rest}) => ({...rest})));
        ...
        return await Promise.all(
            courses.map(async ({tags, coursename}) => {
            const courseId = coursesRows.find(({coursename: name}) => coursename === name).id;
            const tagsId = categoryRows.reduce((acc, {tag, id}) => tags.some(el => el === tag) ? [...acc, id] : acc, [])
            console.log(tagsId);
